...
Simon Kennedy <code@sffjunkie.co.uk>
    - Updated to skip elements before an rdf:RDF is encountered
    - Moved creation of self.stack into startDocument to enable an instance
      to process more than 1 RDF/XML document
"""
...
class RDFParser(xml.sax.handler.ContentHandler): 
    ...
    def emptyPropertyElt(self, e): 
        if e.attrs._elements() in ([], [rdf.ID]): 
            ...
            for a in filter(lambda n: n not in self.disallowed, e.attrs._elements()): 
                if a != rdf.type: 
